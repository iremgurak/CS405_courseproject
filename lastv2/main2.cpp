...
int main()
{
    ...
    /*GameObj3D *floor = new GameObj3D(cubeModel1);
    ...
    /*GameObj3D* donutrectangle1 = new GameObj3D(donutModel);
    donutrectangle1->translate(12, 0, -97.0);
    donutrectangle1->scale(0.2, 0.2, 0.2);
    donutrectangle1->rotate(90, 90, 0);
    donutrectangle1->textureId = 3;
    donutrectangle1->hasGravity = false;
    donutrectangle1->isCollid = false;
    scene.push_back(donutrectangle1);*/
    ...
    waterShader.createShader("lastv2/shader/waterShaderVert.glsl", "lastv2/shader/waterShaderFrag.glsl");
    ...
    const vector<string> texture_files{"lastv2/textures/lava.jpg", "lastv2/textures/darkgrey.jpg", "lastv2/textures/straight-gold.png", "lastv2/textures/donuts.jpg", "lastv2/textures/base-black.png",/*5 textures for cubes*/"lastv2/textures/grey.jpg" };
    ...
    vector<std::string> faces{
        "lastv2/nebula/right.png",
        "lastv2/nebula/left.png",
        "lastv2/nebula/top.png",
        "lastv2/nebula/bottom.png",
        "lastv2/nebula/front.png",
        "lastv2/nebula/back.png"};
    unsigned cubemapTexture = loadCubemap(faces);
    ShaderProgram skyboxShader("lastv2/shader/skybox.vert", "lastv2/shader/skybox.frag");
    skyboxShader.use();
    ...
    // create shader
    ShaderProgram sp("lastv2/shader/vertex.vert", "lastv2/shader/frag.frag");
    sp.use();
    ...
    //ADDED FOR SHADOWS
    Shader2 simpleDepthShader("lastv2/shader/shadow.vs", "lastv2/shader/shadow.fr", "lastv2/shader/shadow.gs");
    //
...
//#endif
    ...
    // ----------------------------
    Shader2 shader("lastv2/shader/text.vs", "lastv2/shader/text.fr");
    ...
    // --------
    FT_Library ft;
    //alttaki satýrý ben ekledim alternatif
    //FT_Init_FreeType(&ft);
    ...
    // find path to font
    string font_name = "lastv2/Zabdilus.ttf";
    if (font_name.empty())
    {
    ...
    while (!Window::isClosed())
    {
        ...
        //colllision with donut or sphere
        for (int i = 0; i < scene.size(); i++) {
            ...
            if (hero.id != obj->id && obj->id != water->id) {
                ...
                if (intersect(hero, *obj) && obj->model3d == &donutModel && obj->textureId == 2) {
                    ...
                    hero_lives -= 1;
                    cout << "Remainin lives of hero:" << hero_lives << endl;
                }
        ...
        //collision için baþka bir deneme lavlar için
        //if (CollidesWithSth(hero)){
        //
        //    //GameObj3D* collidedObj = CollidesWithSthObj(hero);
        //    if (CollidesWithSthObj(hero)->model3d != &sphereModel)
        //    {
        //        if (CollidesWithSthObj(hero)->model3d != &donutModel)
        //        {
        //            DeleteFromScene(CollidesWithSthObj(hero)->id);
        //        }
        //        else if (CollidesWithSthObj(hero)->model3d != &cubeModellava)
        //        {
        //            hero_lives -= 1;
        //            cout << hero_lives << endl;
        //        }
        //    }
        //}
        //
        ...
        //cout << setprecision(100) << float(hero.position().y) << endl;

        if (hero.position().y > -0.7 && hero.position().y <= -0.6)
        {
            hero_lives -= 30;
            cout << "Remaining lives of hero:" << hero_lives << endl;
            hero.front() = glm::vec3(0, 0, -1);
            hero.transformInfo.rotation = glm::vec3(0);
            hero.rotate(hero.rotation().x, hero.rotation().y, hero.rotation().z);
            hero.translate(0, 0.0, 10.0);
        ...
        if (gameContinues == false && game_won == false)
        {
            ...
            //    GameObj3D* donut_obj = scene_donuts[i];
            //    //scene_donuts.pop_back();
            //    scene_donuts.erase(scene.begin() + i);
        ...
        /*else if (gameContinues == false && game_won == false)
        {
            gameContinues = true;
            game_won = false;
            hero_lives = 100;
        ...
        RenderText(shader, str2, 540.0f, 570.0f, 1.0f, glm::vec3(0.5, 0.8f, 0.2f));
        // update the scene
        Window::refresh();
        //cout << hero.position().x << "  " << hero.position().z << setprecision(100) <<"   "<< hero.position().y << endl;
